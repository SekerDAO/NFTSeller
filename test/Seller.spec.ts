...
const etherValueBatch = ethers.utils.parseEther("0.75");
...
describe("Seller", async () => {
  const baseSetup = deployments.createFixture(async () => {
    ...
    const SELLER = await hre.ethers.getContractFactory("Seller");
    const seller = await SELLER.deploy(nft.address, user1.address);
  ...
  describe("NFT", async () => {
    it("should mint NFT", async () => {
      const { nft } = await baseSetup();
      let total = await nft.totalSupply()
      expect(total).to.equal(100);
    });

    it("approve NFTs to seller", async () => {
      const { nft, seller } = await baseSetup();
      await nft.setApprovalForAll(seller.address, true);
      let isApproved = await nft.isApprovedForAll(user1.address, seller.address);
    ...
    it("can mint", async () => {
      const { nft, seller } = await baseSetup();
      await nft.setApprovalForAll(seller.address, true);
      await seller.connect(user2).mint(5, {value: etherValueBatch})
      const balance = await nft.balanceOf(user2.address);
      expect(balance).to.equal(5);
      const minted = await seller.minted();
      expect(minted).to.equal(5);
      const issuerBalance = await nft.balanceOf(user1.address);
      expect(issuerBalance).to.equal(95);
    });
